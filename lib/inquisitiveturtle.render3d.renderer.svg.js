...
inquisitiveturtle.render3d.renderer.svg.prototype.render = function( face, viewid ) {
    if (!this.toRender[this.index])
    {
      this.toRender[this.index] = {
    ...
    {
      this.toRender[this.index].view = face.views[viewid];
      this.toRender[this.index].color = this.lighting.lightSurface(face.getColor(), face.normal, this.toRender[this.index].color);
      this.toRender[this.index].unused = false;
    }
    this.index++;
