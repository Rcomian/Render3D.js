...
inquisitiveturtle.render3d.prim.camera = function() {
    ...
    this._faces = [
      new this.__face().setPoints([ this._points[1], this._points[2], this._points[3] ]).setColor( new this.__vector(1, 0, 0) ),                     // 0 Left plane
      new this.__face().setPoints([ this._points[1], this._points[5], this._points[4] ]).setColor( new this.__vector(0, 1, 0) ),                     // 1 Right plane
      new this.__face().setPoints([ this._points[1], this._points[3], this._points[5] ]).setColor( new this.__vector(0, 0, 1) ),                     // 2 Top plane
      new this.__face().setPoints([ this._points[1], this._points[4], this._points[2] ]).setColor( new this.__vector(1, 1, 0) ),                     // 3 Bottom plane
      new this.__face().setPoints([ this._points[2], this._points[3], this._points[5], this._points[4] ]).setColor( new this.__vector(0, 1, 1) )     // 4 Front plane
    ];
...
inquisitiveturtle.render3d.prim.camera.prototype.extractFaces = function( prims, renderer ) {
    ...
    var primfaces, primface, facepoints;
    ...
    for (var i = 0, ii = prims.length; i < ii; i++)
    {
      ...
      for (var k = 0, kk = prim.getBoundingBoxFaceCount().length; k < kk; k++)
      {
        ...
        worldboundingboxface.resetSpares();
        if (leftclip == 1)
        {
      ...
      for (var j = 0, jj = primfaces.length; j < jj; j++)
      {
        ...
        {
          clipped = [];
          for (k = 0, kk = facepoints.length; k < kk; k++)
          {
          ...
          if (clipped.length > 0) // Check face not clipped away
          {
            ...
            maxcamdepth = 0;
            for (k = 0, kk = clipped.length; k < kk; k++)
            {
