...
inquisitiveturtle.render3d.vector.prototype.clone = function() {
    ...
    var caller;
    try {(0)()} catch (e) {
          caller = e.stack.replace(/^.*?\n/,'').
           replace(/(?:\n@:0)?\s+$/m,'').
           replace(/^\(/gm,'{anonymous}(').
           split("\n")[0];
    }
    
    if (inquisitiveturtle.render3d.vector.clonecallers[caller] == null)
      inquisitiveturtle.render3d.vector.clonecallers[caller] = 1;
    else
      inquisitiveturtle.render3d.vector.clonecallers[caller]++;
    */
...
inquisitiveturtle.render3d.vector.prototype.rotate = function(vrot) {
    ...
    if (vrot.z != 0)
    {
        ...
        this.x = (xt * cosrotz) - (this.y * sinrotz);
        this.y = (xt * sinrotz) + (this.y * cosrotz);
    }
    return this;
  }
